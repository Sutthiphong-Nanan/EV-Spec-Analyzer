...
P_inverter_elec_point_withPV = None
P_inverter_elec_point_with_outPV = None
    ...
    try:
        ...
        def plot_graph_csv(x, y, x_name, y_name, title, label_graph):
            ...
            def save_high_quality():
                    ...
                    def save_file():
                        save_path = filedialog.asksaveasfilename(defaultextension='.png', filetypes=[("PNG files", "*.png"), ("JPEG files", "*.jpg"), ("PDF files", "*.pdf"), ("EPS file", "*.eps"), ("PS file", "*.ps"),
                                                                                                     ("RGBA file", "*rgba"), ("SVG file", "*.svg"), ("TIF file", "*.tif"), ("WEBP file", "*.webp")])
                        if save_path:
...
def calculation(data_route): #กดปุ่ม calculation
    ...
    global P_inverter_elec_point_withPV  
    global P_inverter_elec_point_with_outPV 
    ...
    def power_calculate(F, v, radius, R, eff_mt, eff_inv):
        ...
        power = ((2 * np.pi * N_motor * T_motor) / 60 ) * ( 1 / eff_inv) * ( 1 / eff_mt)
        return power
        

    #คำนวนพลังงานของแต่ละแรง    
    F_acc_power_point = power_calculate(F_acc_point, v_t_point, radius, R, eff_motor, eff_inverter)
    F_rolling_power_point = power_calculate(F_rolling_point, v_t_point, radius, R, eff_motor, eff_inverter)
    F_grade_power_point = power_calculate(F_grade_point, v_t_point, radius, R, eff_motor, eff_inverter)
    F_air_power_point = power_calculate(F_air_point, v_t_point, radius, R, eff_motor, eff_inverter)
    
     #คำนวนพลังงานไฟฟ้า จากทางกล ไม่มี PV
    P_moter_elec_point_with_outPV = P_moter_mech_point * ( 1 / eff_motor) #พลังงานไฟฟ้าที่ใช้จากมอเตอร์
    P_inverter_elec_point_with_outPV = P_moter_elec_point_with_outPV * ( 1 / eff_inverter) #พลังงานไฟฟ้าที่ใช้จากอินเวอร์เตอร์ คำนวณแบตที่ไม่มี PV
    Net_power_point_max_with_outPV = max(P_inverter_elec_point_with_outPV) / 1000 #ค่าพลังงานสุทธิ กรณีไม่มี PV kW

    #คำนวนพลังงานไฟฟ้า จากทางกล มี PV
    P_moter_elec_point_withPV = (P_moter_mech_point) * ( 1 / eff_motor)  #พลังงานไฟฟ้าที่ใช้จากมอเตอร์
    P_inverter_elec_point_withPV = ((P_moter_elec_point_withPV) * ( 1 / eff_inverter)) - (power_PV_run_point) #พลังงานไฟฟ้าที่ใช้จากอินเวอร์เตอร์ คำนวณแบตที่มี PV
    Net_power_point_max_withPV = max(P_inverter_elec_point_withPV) / 1000 #ค่าพลังงานสุทธิ กรณีมี PV kW

    plot_graph(time_point, P_inverter_elec_point_with_outPV , 'Time (sec)', 'Power (W)', 'Power vs Time Graph', 'EV Power', None, None, None) #plot graph    
    ...
    Total_power_nonPV = (np.trapezoid(P_inverter_elec_point_with_outPV, time_point)) * run_turn  #trapezoidal(y, x) คำนวนพื้นที่ใต้กราฟ
    ...
    Total_power_with_PV = (np.trapezoid(P_inverter_elec_point_withPV, time_point)) * run_turn  #trapezoidal(y, x) คำนวนพื้นที่ใต้กราฟ
    Battery_size_with_PV = ((abs(Total_power_with_PV) / 3600) / 1000) * 1.1   #หน่วยเป็น kWh
    ...
    Time_Full_charge_with_PV = (( Battery_size_with_PV / (pv_power_run_charge / 1000) ) * 60) * 60 #ขนาดกำลังไฟฟ้าในการชาร์จแบตเตอรี่เต็มด้วยโซล่าเชลล์ 
    ...
    Label_Battery_size = tkb.Label(scrollframe_tab2_info_result_scroll, text ='Battery Size with PV: {:,.3f} kWh'.format(float(Battery_size_with_PV)))
    Label_Battery_size.grid(row=1, column=1, sticky="w", padx=10)

    Label_Battery_weight_nonPV = tkb.Label(scrollframe_tab2_info_result_scroll, text ='Battery Weight without PV: {:,.3f} kg'.format(float(Battery_weight_nonPV)))
    Label_Battery_weight_nonPV.grid(row=2, column=1, sticky="w", padx=10)
    
    Label_Battery_weight_with_PV = tkb.Label(scrollframe_tab2_info_result_scroll, text ='Battery Weight with PV: {:,.3f} kg'.format(float(Battery_weight_with_PV)))
    Label_Battery_weight_with_PV.grid(row=3, column=1, sticky="w", padx=10)
    
    Label_Net_power_with_outPV = tkb.Label(scrollframe_tab2_info_result_scroll, text = 'Rated Discharge without PV: {:,.3f} kW'.format(float(Net_power_point_max_with_outPV)))
    Label_Net_power_with_outPV.grid(row=4, column=1, sticky="w", padx=10)
    ...
    ToolTip(Label_Net_power_with_outPV, text="Battery Power Output Specification without PV", bootstyle=('dark', tkb.INVERSE))
    ToolTip(Label_Net_power_withPV, text="Battery Power Output Specification with PV", bootstyle=('dark', tkb.INVERSE))
    ToolTip(Label_PV_stop_charge, text="Solar Charging Energy (Stationary)", bootstyle=('dark', tkb.INVERSE))
...
def plot_graph(x, y, x_name, y_name, title, label_graph, etc_X, etc_Y,etc_status): #plot graph
    ...
    if etc_status != None:
        if etc_Y != None and etc_X == None:  # กรณีมีข้อมูล Y เพิ่มเติม
            ...
            if etc_status[4] == True and etc_Y[4] is not None:
                ...
                plot1.plot(x, etc_Y[5], label='Net Power',
                        color='#66CDAA', linewidth=1.5, alpha=0.7)  #สีมิ้น
        ...
        if etc_X != None and etc_Y == None:  # กรณีมีข้อมูล X เพิ่มเติม
            ...
            if etc_status[4] == True and etc_X[4] is not None:
                ...
                plot1.plot(etc_X[5], y, label='Net Power',
                        color='66CDAA', linewidth=1.5, alpha=0.7)  
    ...
    def save_csv():
        ...
        dataframe_result  = pd.DataFrame({"time_point": time_point,
                                        ...
                                        "(Acceleration in Graph)F_acc_power_point": F_acc_power_point,
                                        "(Rolling Resistance in Graph)F_rolling_power_point": F_rolling_power_point,
                                        "(Grade Resistance in Graph)F_grade_power_point": F_grade_power_point,
                                        "(Air Resistance in Graph)F_air_power_point": F_air_power_point,                                 
                                        ...
                                        "(Net_Power)P_inverter_elec_point_withPV": P_inverter_elec_point_withPV,
                                        "(EV_Power)P_inverter_elec_point_with_outPV": P_inverter_elec_point_with_outPV})
...
def plot_config_update(*args):
    if (P_inverter_elec_point_with_outPV is None) or (F_tractive_point is None) or (acc_point is None) or (v_t_point is None) or (position_point is None) or (time_point is None): #ตรวจสอบว่าคำนวนข้อมูลหรือยัง
        messagebox.showerror(message='No data available to display. Please calculate the data first.')
        return
    plot_config = {'power': [P_inverter_elec_point_with_outPV ,'Power (W)', [F_acc_power_point, F_rolling_power_point, F_grade_power_point, F_air_power_point, power_PV_run_point, P_inverter_elec_point_withPV]], 'force': [F_tractive_point,'Force (N)',[F_acc_point, F_rolling_point, F_grade_point, F_air_point, None]], 'acceleration': [acc_point,'Acceleration (m/s²)', None]
    ...
    plot_graph(selected_value_X[0],
                ...
                f'EV {title_dic[selected_value_Y[1]]}',
                selected_value_X[2],
                selected_value_Y[2],
...
button_plot_graph = tkb.Button(frame_tab2_plot, text='Show Graph', bootstyle="success", command=plot_config_update)
button_plot_graph.grid(row=3, column=4, rowspan=3, columnspan=3, padx=5, pady=5, ipadx=50, ipady=20)
...
checkbutton_invert.grid(row=0, column=6, sticky='w', padx=2)
